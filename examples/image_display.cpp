...
void test_src_display()
{
  ...
  std::string config_src = R"(
          ...
          - name: Image1
            ...
            filter_settings:
              timeout: 40000
              folder path: ../../examples/data/tif
)";

  //  Define YAML data inline as a string
  std::string config_src1 = R"(
          Filters:
          - name: Image1
            type: ImgSrc
            filter_settings:
  ...
  YAML::Node cfg_src1 = YAML::Load(config_src1);
  ...
  ImgSrc *src1 = new ImgSrc(cfg_src1["Filters"][0]);
  ...
  pipe.connect(src1, 0, disp, 1);

  pipe.assignTask(0, src);
  pipe.assignTask(1, src1);
